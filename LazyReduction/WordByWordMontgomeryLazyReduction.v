...
Module WordByWordMontgomery.
  ...
  := dlet res := (red_loop num_it (Positional.extend_to_length (2 * N_numlimbs) (2 * N_numlimbs + 1) A)) in
    res.
End red_loop. 
    ...
    Lemma small_exten: forall n1 n2 (v : T n2), (n2 <= n1)%nat -> small v -> @small n1 (Positional.extend_to_length n2 n1 v).
    Proof. intros. Search small. apply small_canon_rep with (x := (eval v)). split.
      - assert (n1 = n2 + (n1 - n2)) by auto with zarith. rewrite H1. rewrite Partition_long.
  ...
  Check SOS_red_add. Local Open Scope Z_scope.
    ...
    Proof.
      unfold small, Partition.partition. simpl. auto.
    Qed.
...
Check first_n.
  ...
  Lemma small_red_loop: forall init count A, small A -> (R_numlimbs <= init)%nat -> (length (A) = init + count + 1)%nat -> small (red_loop (init) count A).
  Proof.
